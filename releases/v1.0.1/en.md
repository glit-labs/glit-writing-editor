...
| Show in Finder (macOS) / Show in File Explorer (Windows) | Reveal the item in the OS file manager |
...
| Dialogue Style | Normalises dialogue formatting to the `"Dialogue"` style |

**Punctuation Correction** *(Korean only)*
...
- Removed the GitHub releases-based self-update checker (automatic banner and manual check button). Both macOS and Windows are now distributed exclusively through the Mac App Store and Microsoft Store respectively; updates are handled automatically by each store.
- Added MSIX (`.appx`) build target for Microsoft Store distribution.
- Added custom tile images (`assets/appx/`) for Microsoft Store certification.
