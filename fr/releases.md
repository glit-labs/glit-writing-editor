---
description: "Notes de version de chaque mise à jour de Glit, classées par plateforme : macOS, Windows, iOS et iPadOS. Découvrez les nouveautés de l'application d'écriture."
layout: single
title: "Versions"
lang: fr
locale: "fr-FR"
toc: false
---

Notes de version de chaque version de Glit, organisées par plateforme. Chaque entrée renvoie aux notes complètes en English et 한국어. Pour une vue chronologique unique sur toutes les plateformes, consultez le [Journal des modifications](/fr/CHANGELOG.html).

<nav class="section-nav" aria-label="Aller à la plateforme">
  <a href="#macos">macOS</a>
  <a href="#windows">Windows</a>
  <a href="#ios--ipados">iOS / iPadOS</a>
  <a href="#android">Android</a>
</nav>

## macOS

Application universelle pour Mac Apple Silicon et Intel, disponible sur le [Mac App Store](https://apps.apple.com/us/app/glit-web-novel-writing-editor/id6764386721).

| Version | Date de sortie | Points clés | Notes de version |
| :------ | :------------- | :---------- | :--------------- |
| **v1.2.1** | 2026-07-02 | Refonte des thèmes et corrections | [English](/releases/mac/v1.2.1/en.html) / [한국어](/releases/mac/v1.2.1/ko.html) |
| v1.1.1 | 2026-06-03 | Corrections et améliorations | [English](/releases/mac/v1.1.1/en.html) / [한국어](/releases/mac/v1.1.1/ko.html) |
| v1.1.0 | 2026-05-31 | Mise à jour de fonctionnalités | [English](/releases/mac/v1.1.0/en.html) / [한국어](/releases/mac/v1.1.0/ko.html) |
| v1.0.1 | 2026-05 | Maintenance | [English](/releases/mac/v1.0.1/en.html) / [한국어](/releases/mac/v1.0.1/ko.html) |
| v1.0.0 | 2026-04 | Version initiale | [English](/releases/mac/v1.0.0/en.html) / [한국어](/releases/mac/v1.0.0/ko.html) |

## Windows

Disponible sur le [Microsoft Store](https://apps.microsoft.com/detail/9n19p02fc7rf) pour Windows 10 et 11.

| Version | Date de sortie | Points clés | Notes de version |
| :------ | :------------- | :---------- | :--------------- |
| **v1.2.1** | 2026-06-30 | Correction de la mise en page des réglages | [English](/releases/windows/v1.2.1/en.html) / [한국어](/releases/windows/v1.2.1/ko.html) |
| v1.2.0 | 2026-06-30 | Refonte des thèmes et corrections | [English](/releases/windows/v1.2.0/en.html) / [한국어](/releases/windows/v1.2.0/ko.html) |
| v1.1.1 | 2026-06-03 | Corrections et améliorations | [English](/releases/windows/v1.1.1/en.html) / [한국어](/releases/windows/v1.1.1/ko.html) |
| v1.1.0 | 2026-05-31 | Mise à jour de fonctionnalités | [English](/releases/windows/v1.1.0/en.html) / [한국어](/releases/windows/v1.1.0/ko.html) |
| v1.0.1 | 2026-05 | Version initiale | [English](/releases/windows/v1.0.1/en.html) / [한국어](/releases/windows/v1.0.1/ko.html) |

## iOS / iPadOS

Application universelle pour iPhone et iPad, disponible sur l'[App Store](https://apps.apple.com/app/glit-web-novel-writing/id6783972543). Prend en charge la synchronisation cloud directe avec Google Drive, Dropbox et OneDrive.

| Version | Date de sortie | Points clés | Notes de version |
| :------ | :------------- | :---------- | :--------------- |
| **v1.1.0** | 2026-07-05 | Déplacer les projets vers/depuis le cloud | [English](/releases/ios_ipados/v1.1.0/en.html) / [한국어](/releases/ios_ipados/v1.1.0/ko.html) |
| v1.0.0 | 2026-07-01 | Version initiale | [English](/releases/ios_ipados/v1.0.0/en.html) / [한국어](/releases/ios_ipados/v1.0.0/ko.html) |

## Android

**Bientôt disponible.** Glit pour Android est en développement. Consultez la [page Android](/releases/android/) pour en savoir plus, ou procurez-vous Glit dès aujourd'hui sur macOS, Windows, iPhone ou iPad depuis la [Boutique](/fr/store.html).
