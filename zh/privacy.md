---
description: "阅读 Glit：写作编辑器的隐私政策，了解你的数据在 macOS、Windows、iPhone 和 iPad 上是如何处理的。"
layout: single
title: "隐私政策"
lang: zh
locale: "zh-CN"
toc: false
---

最后更新：2026-08-11

Glit：写作编辑器（“Glit”“我们”）是一款面向网络小说创作者的写作应用。本政策说明在应用的所有版本（macOS、Windows、iPhone 和 iPad）以及本网站中如何处理你的数据。

<div class="info-grid">
  <div class="info-card">
    <h3>应用不收集任何数据</h3>
    <p>Glit 不运营任何服务器，也不收集个人信息。应用中没有分析、广告或追踪，也无需任何账户。本网站会统计访问量，详见下文。</p>
  </div>
  <div class="info-card">
    <h3>文件始终归你所有</h3>
    <p>你创建的一切都作为由你掌控的普通文件本地存储在你的设备上。</p>
  </div>
  <div class="info-card">
    <h3>云端为可选项</h3>
    <p>Glit 可完全离线运行。云同步完全由你选择，并使用服务商自身的安全登录。</p>
  </div>
</div>

## 应用不收集任何数据
我们不运营自有服务器，也不收集、存储或分享任何个人信息。Glit 不含分析、广告或追踪软件。使用本应用无需账户或注册。

## 文件存储在你的设备上
你创建或编辑的所有项目和文件都本地存储在你的设备上。

## 云存储（可选）
Glit 从不要求使用云存储，可完全离线运行。若你选择使用，其运作方式取决于你的平台：

**在 macOS 和 Windows 上：** 你通过安装云服务商自身的桌面应用（如 Google Drive、Dropbox 或 OneDrive）来同步作品，该应用会在你的电脑上创建一个同步文件夹。Glit 只是像对待其他本地文件夹一样，在该文件夹中打开和保存文件。Glit 本身不会连接云服务，也不会访问你的账户。

**在 iPhone 和 iPad 上：** Glit 通过官方 API 直接连接 Google Drive、Dropbox 或 Microsoft OneDrive，以同步你选择的文件。你通过服务的安全 OAuth 流程登录；你的授权令牌安全地存储在你的设备上，绝不会发送给我们。Glit 只访问同步作品所需的文件，绝不访问你的账户密码。

## 第三方服务
当你使用云服务时（在任何平台上），该服务受其自身隐私政策约束：

- Google（Google 登录、Google Drive）：<https://policies.google.com/privacy>
- Dropbox：<https://www.dropbox.com/privacy>
- Microsoft（OneDrive）：<https://privacy.microsoft.com/privacystatement>

## 网站分析
本节仅适用于本网站。Glit 应用本身仍然不收集任何信息。

我们使用 Cloudflare Web Analytics 来了解有多少人访问 glit.app、哪些页面和语言被阅读，以及访客从哪些网站而来。这是我们特意选择的、比常见分析工具更注重隐私的方式：

- **不使用 Cookie。** 不会在你的设备上存储任何内容，也不会为你分配持久标识符。这就是本网站没有 Cookie 同意横幅的原因。
- **收集内容：** 你浏览的页面、将你引导至此的网站、所在国家/地区、设备类型、浏览器和操作系统，以及页面加载速度。
- **不收集内容：** 你的姓名、电子邮件地址、你在应用中书写的任何内容，以及你访问地址中的查询字符串。
- **访问量统计方式：** Cloudflare 会根据你的 IP 地址、浏览器和请求标头生成一次性数值，进行哈希处理后即予丢弃。你的 IP 地址不会被存储。
- **拒绝方式：** 任何内容拦截或反追踪工具都能阻止该信标运行，网站功能不受任何影响。

Cloudflare 对这些数据的处理方式：<https://www.cloudflare.com/privacypolicy/>

## 你的权利
在我们处理个人数据的范围内，你可以要求访问、更正、删除或限制处理，也可以对处理提出异议。请联系 [kr.johng@gmail.com](mailto:kr.johng@gmail.com)。

## 儿童隐私
Glit 并非面向儿童。应用不收集任何信息，上文所述的网站分析也不会识别任何个人。

## 运营者
本网站及 Glit 应用由大韩民国个体工商户 **Glit（글릿）** 运营，代表人为 Dong Hyun Gang（강동현）。

电子邮件：[kr.johng@gmail.com](mailto:kr.johng@gmail.com)——有关本政策的问题，以及行使上述权利的请求，请发送至该邮箱。

完整通信地址与营业执照编号载于[法律声明](/impressum.html)。
